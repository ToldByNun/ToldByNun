...
function renderFeaturedShowcase() {
  ...
  const featuredLinks = buildProjectLinks(featured);
  if (featuredLinks) content.appendChild(featuredLinks);
...
function renderContact() {
  ...
  const github = getUiNode("contact-github");
  if (github && contact.github) {
    github.href = contact.github;
    setUiText("contact-github-text", "github.com/ToldByNun");
  }

  const job2cv = getUiNode("contact-job2cv");
  if (job2cv && contact.job2cv) {
    job2cv.href = contact.job2cv;
    setUiText("contact-job2cv-text", "job2cv.app");
  }
}
